...
@media(min-width:769px){
  ...
  .modal-box{border-radius:16px;max-width:580px;max-height:90vh}
...
// ── FORM EDIT SESION SECRETARIA ─────────────────────────────────────────────
function FormEditSesionSec({ sesion, kinesiologos, pacientes, sesionesRealizadasMap, onSave, onClose }) {
  const pac = pacientes.find(p => p.id === sesion.paciente_id);
  const realizadas = sesionesRealizadasMap[sesion.paciente_id] || 0;
  const restantes = Math.max((pac?.sesiones || 0) - realizadas, 0);
  const [f, setF] = useState({
    kin_id: sesion.kin_id || "",
    realizada: sesion.realizada,
    notas: sesion.notas || "",
  });
  const set = k => e => setF(p => ({ ...p, [k]: e.target.value }));
  return <>
    <div style={{ background: C.surfaceHigh, borderRadius: 12, padding: "14px 16px", marginBottom: 20, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
      <div>
        <div style={{ fontSize: 11, color: C.textMuted, textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 4, fontWeight: 600 }}>Paciente</div>
        <div style={{ fontWeight: 600, fontSize: 14 }}>{pac?.nombre || "—"}</div>
      </div>
      <div>
        <div style={{ fontSize: 11, color: C.textMuted, textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 4, fontWeight: 600 }}>Tratamiento</div>
        <div style={{ fontSize: 13, color: C.textSoft }}>{pac?.tratamiento || "—"}</div>
      </div>
      <div>
        <div style={{ fontSize: 11, color: C.textMuted, textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 4, fontWeight: 600 }}>Fecha</div>
        <div style={{ fontWeight: 600, color: C.accent }}>{fmtDate(sesion.fecha)}</div>
      </div>
      <div>
        <div style={{ fontSize: 11, color: C.textMuted, textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 4, fontWeight: 600 }}>Sesiones restantes</div>
        <div style={{ fontWeight: 800, fontSize: 20, color: restantes <= 2 ? C.red : C.green }}>{restantes}</div>
      </div>
    </div>
    <Field label="Kinesiólogo asignado a esta sesión">
      <select value={f.kin_id} onChange={set("kin_id")}>
        <option value="">— Sin asignar —</option>
        {kinesiologos.map(k => <option key={k.id} value={k.id}>{k.nombre}</option>)}
      </select>
    </Field>
    <Field label="Notas">
      <textarea value={f.notas} onChange={set("notas")} placeholder="Observaciones de la sesión..." rows={3} style={{ resize: "vertical" }} />
    </Field>
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", background: C.greenSoft, borderRadius: 10, border: `1px solid rgba(5,150,105,0.2)`, marginBottom: 20 }}>
      <input type="checkbox" id="sec-realizada" checked={f.realizada} onChange={e => setF(p => ({ ...p, realizada: e.target.checked }))} style={{ width: 18, height: 18, accentColor: C.green }} />
      <label htmlFor="sec-realizada" style={{ textTransform: "none", fontSize: 14, color: C.green, margin: 0, fontWeight: 600 }}>✓ Marcar como realizada</label>
    </div>
    <div style={{ display: "flex", gap: 10 }}>
      <button className="btn btn-ghost" onClick={onClose} style={{ flex: 1 }}>Cancelar</button>
      <button className="btn btn-primary" onClick={() => onSave(f)} style={{ flex: 2 }}>Guardar cambios</button>
...
function SecDashboard({ pacientes, kinesiologos, sesiones, turnos, solicitudes, mensajes, sesionesRealizadasMap, onEditarSesion }) {
  ...
  const sesionesDia = sesiones.filter(s => s.fecha === filtroDia);
  ...
  return <div className="fade">
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
      <div style={{ fontWeight: 700, fontSize: 16 }}>Gestión de sesiones</div>
      <input type="date" value={filtroDia} onChange={e => setFiltroDia(e.target.value)} style={{ maxWidth: 180, fontSize: 13, padding: "8px 12px" }} />
    </div>

    {sesionesDia.length === 0 ? <Empty msg="Sin sesiones para esta fecha" icon="📋" /> : <>
      {/* Desktop tabla */}
      <div className="card pac-table" style={{ overflow: "hidden", marginBottom: 8 }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: C.surfaceHigh }}>
                {["Paciente", "Kinesiólogo", "Tratamiento", "Fecha", "Ses. restantes", "Estado", "Notas", ""].map(h =>
            ...
            <tbody>
              {sesionesDia.map(s => {
                ...
                const realizadas = sesionesRealizadasMap[s.paciente_id] || 0;
                const restantes = Math.max((pac?.sesiones || 0) - realizadas, 0);
                const col = kinColor(s.kin_id);
                return <tr key={s.id} style={{ borderTop: `1px solid ${C.border}`, cursor: "pointer", transition: "background .15s" }}
                  onClick={() => setEditandoSesion(s)}
                  onMouseEnter={e => e.currentTarget.style.background = C.surfaceHigh}
                  onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  <td style={{ padding: "12px 14px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <Avatar nombre={pac?.nombre || "?"} size={30} color={col} />
                      <div style={{ fontWeight: 600, fontSize: 13 }}>{pac?.nombre || "—"}</div>
                    </div>
                  </td>
                  <td style={{ padding: "12px 14px", fontSize: 13, color: C.textSoft }}>{kin?.nombre || "—"}</td>
                  <td style={{ padding: "12px 14px", fontSize: 13, maxWidth: 160 }}>
                    <div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{pac?.tratamiento || "—"}</div>
                  </td>
                  <td style={{ padding: "12px 14px", fontSize: 13, fontWeight: 600, color: C.accent }}>{fmtDate(s.fecha)}</td>
                  <td style={{ padding: "12px 14px", textAlign: "center" }}>
                    <span style={{ fontWeight: 800, fontSize: 16, color: restantes <= 2 ? C.red : C.green }}>{restantes}</span>
                  </td>
                  <td style={{ padding: "12px 14px" }}>
                    <Badge label={s.realizada ? "Realizada" : "Pendiente"} type={s.realizada ? "activo" : "pendiente"} />
                  </td>
                  <td style={{ padding: "12px 14px", fontSize: 12, color: C.textMuted, maxWidth: 180 }}>
                    <div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.notas || <span style={{ fontStyle: "italic" }}>Sin notas</span>}</div>
                  </td>
                  <td style={{ padding: "12px 14px" }}>
                    <button className="btn btn-ghost" style={{ padding: "5px 9px", fontSize: 12 }} onClick={e => { e.stopPropagation(); setEditandoSesion(s); }}>✏️</button>
                  </td>
                </tr>;
              })}
            </tbody>
      ...
      <div className="pac-cards" style={{ display: "none", flexDirection: "column", gap: 10 }}>
        {sesionesDia.map(s => {
          ...
          const restantes = Math.max((pac?.sesiones || 0) - realizadas, 0);
          const col = kinColor(s.kin_id);
          return <div key={s.id} className="card" style={{ padding: "14px 16px" }} onClick={() => setEditandoSesion(s)}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
              <Avatar nombre={pac?.nombre || "?"} size={40} color={col} />
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 14 }}>{pac?.nombre || "—"}</div>
                <div style={{ fontSize: 12, color: C.textMuted }}>{kin?.nombre}</div>
              </div>
              <Badge label={s.realizada ? "Realizada" : "Pendiente"} type={s.realizada ? "activo" : "pendiente"} />
            </div>
            <div style={{ fontSize: 12, color: C.textSoft, marginBottom: 8, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{pac?.tratamiento}</div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontSize: 12, color: C.textMuted }}>
                Ses. restantes: <span style={{ fontWeight: 700, color: restantes <= 2 ? C.red : C.green }}>{restantes}</span>
              </div>
              <button className="btn btn-ghost" style={{ padding: "6px 12px", fontSize: 13 }} onClick={e => { e.stopPropagation(); setEditandoSesion(s); }}>✏️ Editar</button>
            </div>
          </div>;
        })}
      </div>
    ...
    {editandoSesion && <Modal title="Editar sesión" onClose={() => setEditandoSesion(null)} wide>
      <FormEditSesionSec
        sesion={editandoSesion}
        kinesiologos={kinesiologos}
        pacientes={pacientes}
        sesionesRealizadasMap={sesionesRealizadasMap}
        onSave={async f => { await onEditarSesion(editandoSesion.id, f); setEditandoSesion(null); }}
        onClose={() => setEditandoSesion(null)}
      />
    </Modal>}
  </div>;
}
...
function SecSolicitudes({ solicitudes, setSolicitudes, pacientes, kinesiologos, turnos, setTurnos }) {
  ...
  return <div className="fade">
    <PageHeader title="Solicitudes de turno" subtitle={`${pendientes.length} pendientes`} />
    ...
    <div style={{ width: 220, flexShrink: 0, display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {pacientes.map(p => {
        ...
        return <div key={p.id} onClick={() => { setSelPacId(p.id); marcarLeidos(p.id); }} style={{ padding: "11px 13px", borderRadius: 12, border: `1.5px solid ${selPacId === p.id ? C.accent : C.border}`, background: selPacId === p.id ? C.accentSoft : C.surface, cursor: "pointer", transition: "all .2s", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {nl > 0 && <div style={{ background: C.accent, color: "#fff", borderRadius: "50%", width: 20, height: 20, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700 }}>{nl}</div>}
        </div>;
      })}
    </div>
    <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
      {!selPac ? <Empty msg="Seleccioná un paciente" icon="💬" /> : <>
        ...
        <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, marginBottom: 14 }}>
          ...
          {conv.map(m => {
            ...
            return <div key={m.id} style={{ display: "flex", justifyContent: esSec ? "flex-end" : "flex-start" }}>
              <div style={{ maxWidth: "75%", padding: "10px 14px", borderRadius: esSec ? "16px 16px 4px 16px" : "16px 16px 16px 4px", background: esSec ? C.accent : C.surface, color: esSec ? "#fff" : C.text, fontSize: 14, lineHeight: 1.5, border: esSec ? "none" : `1px solid ${C.border}` }}>
...
function PacTurnos({ paciente, kinesiologos, solicitudes, setSolicitudes }) {
  ...
  const enviarSolicitud = async () => {
    const { data } = await supabase.from("solicitudes_turno").insert([{ paciente_id: paciente.id, kin_id: paciente.kin_id, fecha: f.fecha, hora: f.hora, notas: f.notas, estado: "pendiente" }]).select().single();
  ...
  return <div className="fade">
    ...
    {modal && <Modal title="Solicitar turno" onClose={() => setModal(false)}>
      <div style={{ background: C.accentSoft, border: `1px solid rgba(37,99,235,0.2)`, borderRadius: 10, padding: "12px 14px", marginBottom: 18, fontSize: 13, color: C.accent }}>
        ℹ️ Tu solicitud será revisada por la secretaría.
      </div>
      ...
      <Field label="Notas adicionales"><textarea value={f.notas} onChange={set("notas")} placeholder="Aclaraciones..." rows={3} style={{ resize: "vertical" }} /></Field>
      <div style={{ display: "flex", gap: 10 }}>
...
function PacPerfil({ paciente, onActualizar }) {
  ...
  const set = k => e => setF(p => ({ ...p, [k]: e.target.value }));
  const guardar = async () => { await onActualizar(f); setGuardado(true); setTimeout(() => setGuardado(false), 2000); };
  return <div className="fade">
    ...
    <div className="card" style={{ padding: 22 }}>
      ...
      <button className="btn btn-primary" style={{ width: "100%", marginTop: 8 }} onClick={guardar}>{guardado ? "✓ Guardado" : "Guardar cambios"}</button>
    </div>
  </div>;
}
...
function PacChat({ paciente, mensajes, setMensajes }) {
  ...
  useEffect(() => { bottomRef.current?.scrollIntoView({ behavior: "smooth" }); }, [misMensajes.length]);
  const enviar = async () => {
  ...
  return <div className="fade" style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 160px)" }}>
    ...
    <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, marginBottom: 14 }}>
      {misMensajes.length === 0 && <div style={{ textAlign: "center", color: C.textMuted, padding: 40 }}>
      ...
      {misMensajes.map(m => {
        ...
        return <div key={m.id} style={{ display: "flex", justifyContent: esMio ? "flex-end" : "flex-start" }}>
          <div style={{ maxWidth: "78%", padding: "10px 14px", borderRadius: esMio ? "16px 16px 4px 16px" : "16px 16px 16px 4px", background: esMio ? C.accent : C.surface, color: esMio ? "#fff" : C.text, fontSize: 14, lineHeight: 1.5, border: esMio ? "none" : `1px solid ${C.border}` }}>
            <div>{m.texto}</div>
            <div style={{ fontSize: 10, marginTop: 4, opacity: .65, textAlign: "right" }}>{fmtTime(m.created_at)}</div>
          </div>
        </div>;
...
export default function App() {
  ...
  const [sesionesRealizadasMap, setSesionesRealizadasMap] = useState({});
  ...
  useEffect(() => {
    async function cargar() {
      const [k, p, s, e, t, sol, msg, sesReal] = await Promise.all([
        ...
        supabase.from("sesiones").select("paciente_id").eq("realizada", true),
      ]);
      ...
      const map = (sesReal.data || []).reduce((acc, s) => {
        acc[s.paciente_id] = (acc[s.paciente_id] || 0) + 1;
        return acc;
      }, {});
      setSesionesRealizadasMap(map);
      setLoading(false);
    }
    cargar();
  }, []);
  ...
  const agregarSesion = async f => {
    ...
    const { data } = await supabase.from("sesiones").insert([{ paciente_id: pid, kin_id: pac?.kin_id || null, fecha: f.fecha, notas: f.notas, realizada: f.realizada }]).select().single();
    if (data) {
      ...
      if (f.realizada) {
        ...
        setSesionesRealizadasMap(prev => ({ ...prev, [pid]: (prev[pid] || 0) + 1 }));
      }
    }
  };
  const editarSesion = async (id, f) => {
    const sesionAnterior = sesiones.find(s => s.id === id);
    const { data } = await supabase.from("sesiones").update({ kin_id: Number(f.kin_id) || null, realizada: f.realizada, notas: f.notas }).eq("id", id).select().single();
    if (data) {
      setSesiones(prev => prev.map(s => s.id === id ? data : s));
      const pid = data.paciente_id;
      const pac = pacientes.find(p => p.id === pid);
      if (f.realizada && !sesionAnterior?.realizada) {
        await supabase.from("pacientes").update({ sesiones: (pac?.sesiones || 0) + 1 }).eq("id", pid);
        setPacientes(prev => prev.map(p => p.id === pid ? { ...p, sesiones: p.sesiones + 1 } : p));
        setSesionesRealizadasMap(prev => ({ ...prev, [pid]: (prev[pid] || 0) + 1 }));
      }
      if (!f.realizada && sesionAnterior?.realizada) {
        await supabase.from("pacientes").update({ sesiones: Math.max((pac?.sesiones || 1) - 1, 0) }).eq("id", pid);
        setPacientes(prev => prev.map(p => p.id === pid ? { ...p, sesiones: Math.max(p.sesiones - 1, 0) } : p));
        setSesionesRealizadasMap(prev => ({ ...prev, [pid]: Math.max((prev[pid] || 1) - 1, 0) }));
      }
    }
  };
  ...
  const logout = () => { localStorage.removeItem("kinesioapp_auth"); setAuth(null); setView("home"); setSidebarOpen(false); };
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: C.bg }}>
      ...
      <div className="main-wrap" style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        ...
        <div className="page-pad" style={{ flex: 1, padding: "24px 28px", overflowY: "auto" }}>
          {auth.rol === "secretaria" && view === "home" && <SecDashboard pacientes={pacientes} kinesiologos={kinesiologos} sesiones={sesiones} turnos={turnos} solicitudes={solicitudes} mensajes={mensajes} sesionesRealizadasMap={sesionesRealizadasMap} onEditarSesion={editarSesion} />}
