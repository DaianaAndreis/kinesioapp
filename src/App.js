...
function FormPaciente({ kinesiologos, onSave, onClose, initial = null, fixedKinId = null }) {
  ...
  return <>
    ...
    <Field label="Obra social"><input value={f.obra_social} onChange={set("obra_social")} placeholder="OSDE, Swiss Medical..." /></Field>
...
function FormKinesiologo({ onSave, onClose, initial = null }) {
  ...
  return <>
    ...
    <Field label="Especialidad"><input value={f.especialidad} onChange={set("especialidad")} placeholder="Ej: Traumatología, Deportiva..." /></Field>
    <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
      <button className="btn btn-ghost" onClick={onClose} style={{ flex: 1 }}>Cancelar</button>
        ...
        {turnos.filter(t => t.fecha === hoy).length === 0 ? <Empty msg="Sin turnos para hoy" icon="📅" /> :
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {turnos.filter(t => t.fecha === hoy).sort((a, b) => a.hora.localeCompare(b.hora)).slice(0, 6).map(t => {
              ...
              return <div key={t.id} className="card" style={{ padding: "12px 14px", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {kinesiologos.map(k => {
            ...
            return <div key={k.id} className="card" style={{ padding: "12px 14px", display: "flex", alignItems: "center", gap: 10 }}>
              <Avatar nombre={k.nombre} size={36} color={col} />
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize: 13 }}>{k.nombre}</div>
                <div style={{ fontSize: 11, color: C.textMuted }}>{k.especialidad}</div>
              </div>
              <div style={{ display: "flex", gap: 12, textAlign: "center" }}>
                <div><div style={{ fontWeight: 800, fontSize: 16, color: C.accent }}>{kPac.length}</div><div style={{ fontSize: 10, color: C.textMuted }}>pac.</div></div>
                <div><div style={{ fontWeight: 800, fontSize: 16, color: C.green }}>{kSesHoy.length}</div><div style={{ fontSize: 10, color: C.textMuted }}>ses.</div></div>
              </div>
            </div>;
          })}
        </div>
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr style={{ background: C.surfaceHigh }}>
            {["Paciente", "Kinesiólogo", "Tratamiento", "Ses.", "Estado", ""].map(h => <th key={h} style={{ padding: "11px 14px", textAlign: "left", fontSize: 11, fontWeight: 700, color: C.textMuted, letterSpacing: ".06em", textTransform: "uppercase", whiteSpace: "nowrap" }}>{h}</th>)}
          </tr></thead>
          <tbody>
            ...
            {filtrados.map(p => {
              ...
              return <tr key={p.id} style={{ borderTop: `1px solid ${C.border}` }}>
                ...
                <td style={{ padding: "12px 14px", fontSize: 13, maxWidth: 160 }}><div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.tratamiento}</div></td>
    ...
    <div className="pac-cards" style={{ display: "none", flexDirection: "column", gap: 10 }}>
      ...
      {filtrados.map(p => {
        ...
        return <div key={p.id} className="card" style={{ padding: "14px 16px" }}>
          ...
          <div style={{ fontSize: 13, color: C.textSoft, marginBottom: 8 }}>{p.tratamiento}</div>
          <div style={{ fontSize: 12, color: C.textMuted, marginBottom: 12 }}>{k?.nombre}</div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <Badge label={p.estado} type={p.estado} />
            <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
              <button className="btn btn-ghost" style={{ padding: "6px 12px", fontSize: 13 }} onClick={() => setEditando(p)}>✏️ Editar</button>
              {p.estado === "activo"
...
function KinFichas({ kin, pacientes, setPacientes, sesiones, setSesiones, ejercicios, kinesiologos, onAgregarPaciente, onEditarPaciente, onCambiarEstado, onAgregarSesion, onAgregarEjercicio, onEditarEjercicio, onEliminarEjercicio }) {
  ...
  const [busqueda, setBusqueda] = useState("");
  const [mostrandoLista, setMostrandoLista] = useState(!selId);
  ...
  return <div className="fade">
    {/* Mobile: lista o ficha */}
    <div className="fichas-wrap" style={{ display: "flex", gap: 20 }}>
      {/* Lista */}
      <div className="fichas-list" style={{ width: 220, flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          {pacFiltrados.map(p => <div key={p.id} onClick={() => seleccionarPaciente(p)} style={{ padding: "12px 14px", borderRadius: 12, border: `1.5px solid ${selId === p.id ? col : C.border}`, background: selId === p.id ? `${col}08` : C.surface, cursor: "pointer", transition: "all .2s" }}>
        ...
        {!sel ? <Empty msg="Seleccioná un paciente" icon="👤" /> : <>
          <div className="card" style={{ padding: "16px", marginBottom: 14 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: C.textMuted, marginTop: 2 }}>DNI {sel.dni} · {sel.obra_social || "Sin obra social"}</div>
          ...
          {tab === "info" && <div className="fade card" style={{ padding: 18 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              {[["Tratamiento", sel.tratamiento, "1/-1"], ["Teléfono", sel.tel || "—"], ["Obra social", sel.obra_social || "—"], ["Sesiones", sel.sesiones], ["Inicio", fmtDate(sel.fecha_inicio)], ["Estado", sel.estado]].map(([l, v, col]) =>
          ...
          {tab === "sesiones" && <div className="fade">
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 10 }}><button className="btn btn-primary" onClick={() => setModalSes(true)}>+ Nueva sesión</button></div>
                ...
                {misSesiones.map(s => <div key={s.id} className="card" style={{ padding: "13px 15px" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      ...
                      <button className="btn btn-ghost" style={{ padding: "4px 8px", fontSize: 13 }} onClick={() => setEditandoSesion(s)}>✏️</button>
                    </div>
                  </div>
                  {s.notas && <div style={{ fontSize: 13, color: C.textSoft }}>{s.notas}</div>}
                  {!s.notas && <div style={{ fontSize: 13, color: C.textMuted, fontStyle: "italic" }}>Sin notas</div>}
                </div>)}
              </div>}
          </div>}
                ...
                {misEjercicios.map(e => <div key={e.id} className="card" style={{ padding: "13px 15px" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: e.descripcion ? 8 : 0 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                      ...
                      <button className="btn btn-ghost" style={{ padding: "4px 7px", fontSize: 13 }} onClick={() => setEditEj(e)}>✏️</button>
                      <button className="btn btn-danger" style={{ padding: "4px 7px", fontSize: 13 }} onClick={() => onEliminarEjercicio(e.id)}>🗑</button>
                    </div>
                  </div>
    ...
    <div style={{ maxWidth: 400, width: "100%" }}>
      ...
      <div className="card" style={{ padding: 24 }}>
        ...
        <div style={{ marginTop: 18, padding: 14, background: C.surfaceHigh, borderRadius: 10, fontSize: 12, color: C.textMuted, lineHeight: 1.9 }}>
          ...
          <div>🩺 carolina@kinesio.com / 1234</div>
        </div>
      </div>
    </div>
  </div>;
