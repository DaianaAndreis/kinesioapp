...
.btn-primary{background:${C.accent};color:#fff;box-shadow:0 1px 2px rgba(37,99,235,0.3)}.btn-primary:hover{background:${C.accentHover};transform:translateY(-1px)}
...
.mobile-header{display:none;align-items:center;justify-content:space-between;padding:14px 16px;background:${C.sidebar};color:#fff;position:sticky;top:0;z-index:100;flex-shrink:0}
...
function Badge({ label, type }) {
  ...
  return <span style={{ display: "inline-flex", alignItems: "center", padding: "3px 10px", borderRadius: 20, fontSize: 11.5, fontWeight: 600, color: fg, background: bg, border: `1px solid ${fg}25`, whiteSpace: "nowrap" }}>{label}</span>;
}
...
function FormKinesiologo({ onSave, onClose, initial = null }) {
  ...
  return <>
    ...
    <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 4 }}>
      <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
      <button className="btn btn-primary" disabled={!valid} onClick={() => valid && onSave(f)} style={{ opacity: valid ? 1 : .45 }}>{initial ? "Guardar cambios" : "Agregar kinesiólogo"}</button>
    </div>
  </>;
}
...
function FormTurno({ pacientes, kinesiologos, onSave, onClose, initial = null }) {
  ...
  return <>
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 14px" }} className="form-grid">
      ...
      <Field label="Notas" col="1/-1"><textarea value={f.notas} onChange={set("notas")} placeholder="Observaciones..." rows={2} style={{ resize: "vertical" }} /></Field>
    </div>
    <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 4 }}>
      <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
...
function SecDashboard({ pacientes, kinesiologos, sesiones, turnos }) {
  ...
  return <div className="fade">
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }} className="form-grid">
      ...
      <div>
        <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 14 }}>Por kinesiólogo</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {kinesiologos.map(k => {
          ...
          <tbody>
            ...
            {filtrados.map(p => {
              ...
              return <tr key={p.id} className="table-row" style={{ borderTop: `1px solid ${C.border}` }}>
                ...
                <td style={{ padding: "13px 16px", fontSize: 13, color: C.textSoft, whiteSpace: "nowrap" }} className="hide-mobile">{k?.nombre || "—"}</td>
                <td style={{ padding: "13px 16px", fontSize: 13, maxWidth: 180 }} className="hide-mobile"><div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.tratamiento}</div></td>
...
function SecTurnos({ turnos, pacientes, kinesiologos, onAgregar, onEditar, onEliminar }) {
  ...
  return <div className="fade">
    ...
    <div style={{ display: "flex", gap: 10, marginBottom: 18, flexWrap: "wrap", alignItems: "center" }}>
      ...
      <select value={filtroKin} onChange={e => setFiltroKin(e.target.value)} style={{ maxWidth: 240 }}>
...
function KinHoy({ kin, pacientes, sesiones, turnos, onNuevaSesion }) {
  ...
  return <div className="fade">
    ...
    <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 14 }}>Mis turnos de hoy</div>
        ...
        {tab === "info" && <div className="fade card" style={{ padding: 24 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }} className="form-grid">
            ...
            {sel.notas && <div style={{ gridColumn: "1/-1" }}>
              ...
              <div style={{ fontSize: 13, color: C.textSoft, background: C.surfaceHigh, padding: "12px 14px", borderRadius: 10 }}>{sel.notas}</div>
            </div>}
          </div>
        </div>}
...
function Login({ onLogin }) {
  ...
  return <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #eff6ff 0%, #f0fdf4 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
    <div style={{ maxWidth: 400, width: "100%" }}>
      ...
      <div className="card" style={{ padding: 32 }}>
        ...
        <button className="btn btn-primary" style={{ width: "100%", padding: 14, fontSize: 15 }} onClick={handleLogin} disabled={loading}>{loading ? "Ingresando..." : "Ingresar →"}</button>
        <div style={{ marginTop: 20, padding: 14, background: C.surfaceHigh, borderRadius: 10, fontSize: 12, color: C.textMuted, lineHeight: 1.8 }}>
  ...
  return <>
    ...
    <div id="sidebar" className={isOpen ? "open" : ""} style={{ width: 220, background: C.sidebar, padding: "22px 12px", display: "flex", flexDirection: "column", gap: 4, position: "fixed", top: 0, bottom: 0, zIndex: 200, overflowY: "auto", transition: "left .3s ease" }}>
      ...
      <div style={{ marginTop: "auto", paddingTop: 20 }}>
        ...
        <button className="nav-item" onClick={onLogout} style={{ color: "#94a3b8", fontSize: 12 }}>↩ Cerrar sesión</button>
      </div>
    </div>
  </>;
}
